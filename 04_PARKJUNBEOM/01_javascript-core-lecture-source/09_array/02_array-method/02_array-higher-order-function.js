// 배열 고차 함수 (중요 React에서 또 다시 사용)

// Array.prototype.sort
let numbers = [];
for (let i = 0; i < 10; i++) {
  numbers[i] = Math.floor(Math.random() * 100) + 1; // 1 ~ 100까지 랜덤한 숫자를 뽑아서 numbers에 넣기기
}
console.log(`정렬 전 numbers : ${numbers}`);
numbers.sort(); // 정렬 오름차순순
console.log(`정렬 후 numbers : ${numbers}`);

// sort의 기본 정렬은 문자 정렬이므로 필요하다면 정렬기준 콜백 함수를 전달해야한다.
numbers.sort((a, b) => a - b); // 콜백 함수
console.log(`숫자 오름차순 정렬 후 numbers : ${numbers}`);
// 숫자 내림차순 정렬
numbers.sort((a, b) => b - a); // 콜백 함수
console.log(`숫자 내림차순 정렬 후 numbers : ${numbers}`);

// Array.prototype.forEach : for를 대체할 수 있는 고차함수 (**중요**)
numbers = [1, 2, 3, 4, 5];
// numbeers.forEach((item, index, array) => {
...
result = [1, 5, 3, 2, 4].every((item) => item > 3); // 모든 내용이 3보다 커야 true
console.log(`result : ${result}`);

result = [1, 5, 3, 2, 4].every((item) => item > 0);
console.log(`result : ${result}`);

// Array.prototype.find : 배열을 순회하며 각 요소에 대하여 인자로 주어진 콜백함수를 실행하여 그 결과가 참인 첫번째요소를 반환
// 참인 요소가 존재하지 않는다면 undefined를 반환
// Array.prototype.findIndex : 배열을 순회하며 각 요소에 대하여 인자로 주어진 콜백함수를 실행하여 그 결과가 참인첫번째 요소의 인덱스를 반환
...
console.log("result : ", result);
result = students.findIndex((item) => item.name === "홍길동동");
console.log("result :", result);
// 배열 반환
result = students.filter((item) => item.score >= 60);
console.log("60점 이상인 사람들 result :", result);
